package jp.atziluth.net{
	...
	public class XMLLoader extends EventDispatcher {
		...
		public static const JIS:String = "iso-2022-jp";
		public var onLoadComplete:Function;
		public var onLoadIOError:Function;
		public var onLoadSecurityError:Function;
		public function load(xmlURL:String, charset:String="utf-8"):void {
			xmlLoader = new URLLoader();
			urlRequest = new URLRequest(xmlURL);
			$charset = charset;
			xmlLoader.addEventListener(Event.COMPLETE, onXMLLoadComplete);
			xmlLoader.addEventListener(IOErrorEvent.IO_ERROR, onXMLLoadIOError);
			xmlLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR, onXMLLoadSecurityError);
			xmlLoader.dataFormat = ($charset == UTF_8) ? URLLoaderDataFormat.TEXT : URLLoaderDataFormat.BINARY;
			xmlLoader.load(urlRequest);
		}
		private function onXMLLoadComplete(event:Event):void {
			items = event.target.data;
			try {
				if (xmlLoader.dataFormat == URLLoaderDataFormat.TEXT) {
					$xml = new XML(items);
				} else {
					byteArray = items as ByteArray;
					if ($charset == EUC_JP) {
						$xml = new XML(Jcode.EUCtoUTF8(byteArray));
					} else {
						$xml = new XML(byteArray.readMultiByte(byteArray.length,$charset));
					}
				}
		...
		private function onXMLLoadIOError(event:IOErrorEvent):void {
			if (onLoadIOError != null) {
				onLoadIOError(event);
			}
			removeEventListeners();
		}
		private function onXMLLoadSecurityError(event:SecurityErrorEvent):void {
			if (onLoadSecurityError != null) {
				onLoadSecurityError(event);
			}
			removeEventListeners();
		}
		private function removeEventListeners():void {
			xmlLoader.removeEventListener(Event.COMPLETE, onXMLLoadComplete);
			xmlLoader.removeEventListener(IOErrorEvent.IO_ERROR, onXMLLoadIOError);
			xmlLoader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, onXMLLoadSecurityError);
		}
...
internal class Jcode {
		...
		while (bytes.bytesAvailable > 0) {
			...
			if (ch == 0) {
				...
				if ((ch >= 0xa1 && ch <= 0xf4) && (c >= 0xa1 && c <= 0xfe)) {
					r+=getCharFromTable(ch-0x80,c-0x80);
				} else if (ch == 0x8e) {
					if (c>=0xa1&&c<=0xdf) {
	...
	public static function getCharFromTable(ch:int, cl:int):String {
		var n:int=-1;
		if (ch>=0x21&&ch<=0x28) {
